...
class Schedule extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Doctor', 'Patient', 'Nurse', 'Receptionist', 'Clerk', 'Midwife'))) {
    ...
    function index() {
        ...
        $data['location_schedules'] = $this->schedule_model->getLocationSchedule();
...
// Validating Week Day Field   
    ...
    function addScheduleForLocation() {
        $redirect = $this->input->post('redirect');
        if (empty($redirect)) {
            $redirect = 'schedule';
        }
        ...
        if (empty($id)) {
            $check = $this->schedule_model->getScheduleByDoctorByWeekdayForLocation($doctor, $weekday, $location);
            if (!empty($check)) {
                $this->session->set_flashdata('error', lang('schedule_already_exists'));
                redirect($redirect);
        ...
        if (!empty($id)) {
            $previous_time = $this->schedule_model->getScheduleByDoctorByWeekdayByIdForLocation($doctor, $weekday, $id, $location);
        } else {
            $previous_time = $this->schedule_model->getScheduleByDoctorByWeekdayForLocation($doctor, $weekday, $location);
        }
...
// Validating Week Day Field   
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($id)) {
                $this->session->set_flashdata('success', lang('record_updated'));
                $this->schedule_model->updateScheduleForLocation($id, $data);
            } else {
                        ...
                        foreach ($all_slot as $key_end => $value_e) {
                            if ($slot_e_time == $key_end) {
                                if ($slot_e_time <= $key2) {
                                    ...
                                    $this->schedule_model->insertTimeSlotForLocation($slot_data);
                                }
                            }
                        }
                        $slot_s_time = $slot_e_time;
                ...
                $this->schedule_model->insertScheduleForLocation($data);
                $this->session->set_flashdata('success', lang('record_added'));
            }
    ...
    function deleteScheduleForLocation() {
        $from = $this->input->get('all');
        $id = $this->input->get('id');

        $schedule_details = $this->schedule_model->getLocationScheduleById($id);
        if ($schedule_details->hospital_id != $this->session->userdata('hospital_id')) {
            redirect('redirect/permission');
        }
        ...
        $weekday = $this->input->get('weekday');
        $location = $this->input->get('location');
        $this->schedule_model->deleteTimeSlotByDoctorByWeekdayForLocation($doctor, $weekday, $location);
        $this->schedule_model->deleteScheduleForLocation($id);
        $this->session->set_flashdata('success', lang('record_deleted'));
...
// Validating Week Day Field   
    ...
    function getAvailableSlotByDoctorByDateByLocationByJason() {
        $data = array();
        $date = $this->input->get('date');
        $location = $this->input->get('location');
        if (!empty($date)) {
