                    ...
                    <a data-toggle="modal" href="#myModal">
                        <div class="btn-group pull-right">
                            <button id="" class="btn btn-primary btn-xs">
                                <i class="fa fa-plus"></i> <?php echo lang('add_new_admission'); ?>
                            </button>
                        </div>
                    </a>
                </div>
                    ...
                    <table class="table table-striped table-hover table-bordered" id="editable-sample">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo lang('admitted_time'); ?></th>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" action="admission/addAdmission" class="clearfix row" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-md-12">
                        <label for="exampleInputEmail1"><?php echo lang('admitted_time'); ?></label>
...
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="editAdmissionForm" action="admission/addAdmission" class="clearfix row" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
    $(document).ready(function () {
        $(".table").on("click", ".editbutton", function () {
            ...
            $('#editAdmissionForm').trigger("reset");
            $('#myModal2').modal('show');
            $.ajax({
                url: 'admission/editAdmissionByJason?id=' + iid,
                method: 'GET',
                data: '',
                dataType: 'json',
            }).success(function (response) {
                // Populate the form fields with the data returned from server
                $('#editAdmissionForm').find('[name="id"]').val(response.admission.id).end()
                $('#editAdmissionForm').find('[name="bed_id"]').val(response.admission.bed_id).end()
              //  $('#editAllotmentForm').find('[name="patient"]').val(response.admission.patient).end()
               var option = new Option(response.patient.name + '-' + response.patient.id, response.patient.id, true, true);
                $('#editAdmissionForm').find('[name="patient"]').append(option).trigger('change');
                $('#editAdmissionForm').find('[name="a_time"]').val(response.admission.a_time).end()
                $('#editAdmissionForm').find('[name="d_time"]').val(response.admission.d_time).end()
            });
        });     
    });
</script>
...
<script>
    ...
    $(document).ready(function () {
        var table = $('#editable-sample').DataTable({
            ...
            "searchable": true,
            "ajax": {
                url: "admission/getAdmissionList",
                type: 'POST',
            },
            scroller: {
