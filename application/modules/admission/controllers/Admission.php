...
class Admission extends MX_Controller {
    ...
    function admissions() {
        ...
        $data['admissions'] = $this->admission_model->getAdmission();
    ...
    function addAdmissionView() {
        $data = array();
        $data['beds'] = $this->bed_model->getBed();
        $data['patients'] = $this->patient_model->getPatient();
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('add_admission_view', $data);
        $this->load->view('home/footer'); // just the header file
    }
    ...
    function addAdmission() {
        ...
        $this->form_validation->set_rules('a_time', 'Admitted Time', 'trim|required|min_length[1]|max_length[100]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $admission_count = $this->admission_model->getAdmissionCount() + 1;
            $data = array(
                'bed_id' => $bed_id,
                'patient' => $patient,
                'a_time' => $a_time,
            ...
            $data2 = array(
                'number' => date('ymd').format_number_with_digits($admission_count,4)
            );
    ...
    function editAdmissionByJason() {
        $id = $this->input->get('id');
        $data['admission'] = $this->admission_model->getAdmissionById($id);
        $data['patient'] = $this->patient_model->getPatientById($data['admission']->patient);
        echo json_encode($data);
    }

    function deleteAdmission() {
        $id = $this->input->get('id');
        $this->bed_model->deleteAdmission($id);
        $this->session->set_flashdata('feedback', lang('deleted'));
    ...
    function getAdmissionList() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['beds'] = $this->admission_model->getAdmissionBySearch($search);
            } else {
                $data['beds'] = $this->admission_model->getAdmission();
            }
        } else {
            if (!empty($search)) {
                $data['beds'] = $this->admission_model->getAdmissionByLimitBySearch($limit, $start, $search);
            } else {
                $data['beds'] = $this->admission_model->getAdmissionByLimit($limit, $start);
            }
        }
        ...
        foreach ($data['beds'] as $bed) {
            ...
            $option2 = '<a class="btn btn-danger btn-xs btn_width delete_button" href="admission/deleteAdmission?id=' . $bed->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"> </i> ' . lang('delete') . '</a>';
        ...
        if (!empty($data['beds'])) {
            $output = array(
                "draw" => intval($requestData['draw']),
                "recordsTotal" => $this->admission_model->getAdmissionCount(),
